...
├── src/
│   ├── main.cpp
│   └── dummy_generator/      # 더미 데이터 생성 모듈 (폴더 단위 이식 가능)
│       ├── CMakeLists.txt
│       ├── dummy_generator.hpp
│       └── dummy_generator.cpp
├── lib/
│   ├── json/             # 자체 JSON 라이브러리
│   ├── persistence/      # JSON 파일 기반 CRUD 데이터 저장소
...
출력물: `build\Release\DummyDataGenerator.exe`
...
:: 커버리지 리포트 출력
llvm-cov report ./build-test/DummyDataGenerator.exe -instr-profile=coverage.profdata

:: HTML 리포트 생성
llvm-cov show ./build-test/DummyDataGenerator.exe -instr-profile=coverage.profdata ^
